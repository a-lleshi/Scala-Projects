...
def get_january_data(symbol: String, year: Int) : List[String] = {
  ...
  val january = lines.filter(line => line.startsWith(year.toString))
  january
}
...
def get_first_price(symbol: String, year: Int) : Option[Double] = {
    val january_lines = get_january_data(symbol, year)
    if (january_lines.isEmpty) None
    else Some(january_lines(0).split(",")(1).toDouble)
}
...
def get_deltas(data: List[List[Option[Double]]]) :  List[List[Option[Double]]] = {
    val deltas = for (i <- 0 until data.size - 1) yield {
        val delta = for (j <- 0 until data(0).size) yield {
            get_delta(data(i)(j), data(i+1)(j))
        }
        delta.toList
    }
    deltas.toList
...
def yearly_yield(data: List[List[Option[Double]]], balance: Long, index: Int) : Long = {
    val stockSizeCal = for (i <- 0 until data(index).size) yield {
        if (data(index)(i).isEmpty) 0
        else data(index)(i).get
    }

    val stockList = stockSizeCal.toList
    val newList = stockList.filter(_ != 0.0)
    val pricePerStock = balance / newList.size

    val yearlyYield = for (i <- 0 until newList.size) yield {
        if (newList(i) == 0) 0
        else pricePerStock*newList(i)
    }

    balance + yearlyYield.toList.sum.toLong
}
